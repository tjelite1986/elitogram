...
// Admin/owner widget on a profile that has an Instagram source connected: pull
// the account's media into this profile (photos -> posts, videos -> shorts).
export default function ProfileInstagramSync({
  handle,
  initial,
}: {
  handle: string;
  ...
  const [status, setStatus] = useState<IgStatus>(initial);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    const r = await fetch(`/api/profiles/${encodeURIComponent(handle)}/instagram-sync`);
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      const r = await fetch(`/api/profiles/${encodeURIComponent(handle)}/instagram-sync`);
      if (!r.ok) return;
      const next: IgStatus = await r.json();
      setStatus(next);
  ...
  const sync = async (mode: "all" | "photos") => {
    setBusy(true);
    setMsg(null);
    const res = await fetch(`/api/profiles/${encodeURIComponent(handle)}/instagram-sync`, {
      method: "POST",
  ...
  // Metadata only: refresh name/bio/avatar/links, no media download.
  const syncInfo = async () => {
    ...
    const d = await res.json().catch(() => ({}));
    if (res.ok) {
      setMsg(d.fetched ? "Profile info updated from Instagram." : "Couldn't fetch info (account private or unavailable).");
      router.refresh();
    } else {
