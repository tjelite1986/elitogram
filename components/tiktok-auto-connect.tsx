...
// Admin: scan post-creator folders and link each to TikTok when the folder name
// is a real TikTok account. Runs in bounded batches; keeps going until there
// are no more to check.
export default function TiktokAutoConnect() {
  ...
  const run = async () => {
    setBusy(true);
    setMsg("Checking folders against TikTok…");
    let connected = 0;
    let checked = 0;
    try {
      // Loop batches until nothing remains (each batch verifies ~60 accounts).
      for (let i = 0; i < 50; i++) {
        const res = await fetch("/api/tiktok/auto-connect", {
          method: "POST",
      ...
      setMsg(`Done — connected ${connected} profile(s) to TikTok (checked ${checked}).`);
      router.refresh();
    } catch {
  ...
  return (
    <div className="mt-3">
      ...
      >
        ...
        Auto-connect folders to TikTok
      </button>
